...
/**
 * Phase 1 integration tests for the auction demo actors: `account` and
 * `auction`. Exercises the state machine, snipe protection, and the
 * direct cross-actor hold/releaseHold path without going through
 * `bidSaga` or `settlementSaga` (those arrive in Phase 2).
 */
...
// Tests that want to *observe* a particular phase without timer ticks
// advancing state use this config. Durations are long enough that
// `drain` (a bounded finishAllScheduledFunctions) completes its ~2s of
// busywork well before the first tick would fire.
const LONG_CONFIG = {
...
// Short config for tests that want to drive the state machine all the
// way to expired / settling via the drain loop.
const SHORT_CONFIG = {
  durationMs: 2_000,
  goingOnceMs: 2_000,
  goingTwiceMs: 2_000,
...
// ── end-to-end: direct bid flow (no saga yet) ───────────────────
